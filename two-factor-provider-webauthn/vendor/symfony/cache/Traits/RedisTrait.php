...
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Connection\Cluster\ClusterInterface as Predis2ClusterInterface;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Connection\Cluster\RedisCluster as Predis2RedisCluster;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Predis\Connection\Replication\ReplicationInterface as Predis2ReplicationInterface;
...
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Relay\Relay;
use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Relay\Sentinel;
...
trait RedisTrait
{
    ...
    public static function createConnection(#[\SensitiveParameter] string $dsn, array $options = []): \Redis|\RedisArray|\RedisCluster|\Predis\ClientInterface|Relay
    {
        ...
        if (isset($params['redis_sentinel']) && !is_a($class, \Predis\Client::class, true) && !class_exists(\RedisSentinel::class) && !class_exists(Sentinel::class)) {
            throw new CacheException(sprintf('Cannot use WildWolf\WordPress\TwoFactorWebAuthn\Vendor\Redis Sentinel: class "%s" does not extend "Predis\Client" and neither ext-redis >= 5.2 nor ext-relay have been found.', $class));
        }
